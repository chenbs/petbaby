...
- `apps/miniprogram/`: native WeChat Mini Program client (page counts are maintained in docs/README.md); keep page `.js/.json/.wxml/.wxss` files complete and register every page in `app.json` (`pages` or `subPackages`).
...
- `docs/product/07-functional-backlog.md`: authoritative remaining feature work; do not mix deployment or credential tasks into it. Its payment compliance and `growth_orders` code workstream is complete; remaining items are external signing, credentials, provider integration and release validation, while the other sections are completion registers.
...
- `docs/product/23-虚拟支付合规改造方案.md`: global virtual-payment compliance work.
...
- **Virtual payment compliance** (`docs/product/23-虚拟支付合规改造方案.md`): WeChat requires all-terminal virtual payment for virtual goods, and **the 2026-04-01 deadline has passed**. The SKU-specific provider routing, payment confirmation, idempotent entitlements, refunds and iOS refund-query audit are implemented; external signing, credentials, real callbacks and device validation remain before release.
- **The current pet-human handoff is locally integrated but not released**: on 2026-08-22 the 40 self-owned V2 WebP effects were inventoried and normalized in `tools/imagegen/out/pet-human-v2/effects/`. Numeric source ID `N` maps to `human-effect-NN`; images are `720x1280`, prompts are normalized in `apps/platform/src/server/pet-human-effect-prompts.json`, and hash-based planned object keys are registered. The old 12 experimental template records are removed. Do not regenerate the effects or reuse retired `humanization-v1` outputs. All V2 entries remain `pending-review`; do not upload to production, seed, freeze, or mark `live` without explicit release approval. Other release priorities remain remote CI execution, gitleaks, Docker deployment rehearsal, real Provider/WeChat/supplier integration, real-device walkthroughs, and the legal opinion on health-line copy.
- **Companion and photo records** (`docs/product/33-陪伴与记录开发清单及实施计划.md`): T00–T09 code and T10 local automation are implemented with forward-only migrations `0030`–`0033`. Normal photos persist without work references; uploads have idempotent receipts, metadata editing and keyset pagination. Originals are owner-only; public deliverables use separate assets, and deletion has persistent cleanup retries. Annual films freeze the selected pet, photos and narrative dates at enqueue time. See `docs/delivery/09-陪伴与记录实施验收记录.md` for A01–A18 evidence, test-isolation incidents and outstanding device/provider/release checks; local automated success is not release approval.
...
In `apps/miniprogram/`, style exclusively through theme tokens — register a new token in `theme/tokens.js` before using it, and supply it in all four skins. Avoid optional chaining and nullish coalescing to stay compatible with base library 2.19.2.
