...
for (const name of names) {
  ...
  for (const width of [160, 320, 640, 960]) {
    ...
    await sharp(source).resize({ width, withoutEnlargement: true }).webp({ quality: 78 }).toFile(output);
    await copyFile(output, path.resolve(import.meta.dirname, "../../../docs/website/prototype/assets", path.basename(output)));
  }
}
