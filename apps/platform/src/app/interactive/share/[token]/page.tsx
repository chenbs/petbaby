...
export default async function InteractiveSharePage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ source?: string }> }) {
  ...
  if (!session) return <main className="share-screen"><section className="empty-state"><div><b>{expired ? "这份互动分享已经失效" : "没有找到这份互动页"}</b><p>{expired ? "主人已撤销分享或有效期已经结束。" : "链接可能不完整。"}</p><Link className="primary-button" href="/">返回麻麻抱我</Link></div></section></main>;
  return <main className="share-screen interactive-share-screen"><header className="share-intro"><span>麻麻抱我 · 互动回忆</span><h1>{String(session.snapshot.title || "一页会回应的回忆")}</h1></header><InteractiveGuestClient session={session} source={query.source || "share"} token={token} /><Link className="primary-button share-cta" href="/interactive/create">给我的宠物也做一页</Link></main>;
}
