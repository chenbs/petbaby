/** Generate 14 cat/dog runtime migrations from the seven frozen third-batch masters. */
...
import {
  buildThirdBatchValidationPrompt,
  thirdBatchValidationJobs
} from "./reference-third-batch-validation-prompts.mjs";
...
const targetIds = TARGET === "all" ? [] : TARGET.split(",").filter(Boolean);
...
const requestedConcurrency = Number(
  concurrencyArgument?.split("=")[1]
    || process.env.REFERENCE_VALIDATION_CONCURRENCY
    || process.env.LINGSUAN_IMAGE_CONCURRENCY
    || 1
);
const CONCURRENCY = Math.max(1, Math.min(20, Number.isFinite(requestedConcurrency) ? Math.floor(requestedConcurrency) : 1));
...
async function generateJob(config, job) {
  ...
  await writeFile(metadataPath, `${JSON.stringify({
    ...
    sceneChangeBudget: "0%",
    queue: { configuredConcurrency: config.concurrency, localWorkerConcurrency: CONCURRENCY, maxRetriesPerTask: 3, maxAttemptsPerTask: 4 },
    maskIncluded: false,
...
if (targetIds.some((id) => !thirdBatchValidationJobs.some((job) => job.id === id))) {
  throw new Error(`未知任务 ${targetIds.filter((id) => !thirdBatchValidationJobs.some((job) => job.id === id)).join(", ")}`);
}
...
const selected = TARGET === "all"
  ? thirdBatchValidationJobs
  : thirdBatchValidationJobs.filter((job) => targetIds.includes(job.id));
await verifyInputs(selected);
const config = await loadEnv();
if (config.concurrency !== CONCURRENCY) {
  throw new Error(`共享队列并发 ${config.concurrency} 与本次任务池 ${CONCURRENCY} 不一致，请使用 --concurrency=${CONCURRENCY}`);
}
...
console.log(`第三批迁移生成结束：成功 ${selected.length - failures.length} 张，失败 ${failures.length} 张`);
if (failures.length) {
